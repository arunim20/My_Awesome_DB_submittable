...
pub fn operator_budget_bytes(memory_limit_mb: u64) -> usize {
    ...
    // Fixed overhead: cache (4MB) + process baseline (binary+stack+libc ~10MB)
    // + bloom filters + HashMap amplification headroom (6MB)
    let fixed_overhead = 20 * 1024 * 1024;
    let available = total_bytes.saturating_sub(fixed_overhead);
    let dynamic = available / (live + 1);
    // Cap at 30% of total — conservative to prevent sort flush peak OOM
    let max_budget = total_bytes * 30 / 100;
